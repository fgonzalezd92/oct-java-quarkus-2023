...
//    Predicate<String> isPasswordLengthValid = value -> value.length() >= 8 && value.length() <= 14;
    ...
    BiFunction<Integer, Integer, Predicate<String>> lengthValidator2 = (Integer min, Integer max) -> {
        Predicate<String> lengthPredicate = (String value) -> value.length() >= min && value.length() <= max;
